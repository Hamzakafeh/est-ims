...
let _usernameAutoGenerated = false;

function _toEnglishSlug(str) {
  // Keep only ASCII letters, digits, hyphens
  return str.replace(/[^A-Za-z0-9\-]/g, '').replace(/^-+|-+$/g, '');
}

function _generateUsername(fullName) {
  const parts = fullName.trim().split(/\s+/).map(p => _toEnglishSlug(p)).filter(Boolean);
  if (!parts.length) return '';
  // Use first or last English part (prefer first)
...
function onFullNameInput(val) {
  // Real-time blocked name check
  if (_isBlockedName(val)) {
    const msg = authLang === 'ar' ? 'هذا الاسم محجوز ولا يمكن التسجيل به' : 'This name is reserved and cannot be used';
    setHint('hintFullName', msg);
    markInvalid(document.getElementById('fullName'));
    return;
  }
  setHint('hintFullName', '');
  const fnEl = document.getElementById('fullName');
  if (fnEl) fnEl.classList.remove('invalid');

  const uInput = document.getElementById('username');
  const tag    = document.getElementById('usernameAutoTag');
  if (!uInput) return;
  if (!_usernameAutoGenerated && uInput.value && !uInput.dataset.wasAuto) return;
  const generated = _generateUsername(val);
  if (generated) {
    uInput.value = generated;
    uInput.dataset.wasAuto = '1';
    _usernameAutoGenerated = true;
    if (tag) tag.style.display = 'inline';
    setHint('hintUsername', '');
  } else {
    if (!val.trim()) { uInput.value = ''; if (tag) tag.style.display = 'none'; _usernameAutoGenerated = false; }
  }
}
...
function onPhoneInput(el) {
  ...
  if (_BLOCKED_PHONES.includes(digits) || _BLOCKED_PHONES.includes(el.value.trim())) {
    markInvalid(el);
    setHint('hintPhone', authLang === 'ar' ? 'رقم الهاتف هذا محظور' : 'This phone number is not allowed');
  } else {
...
function onUsernameManualEdit() {
  const uInput = document.getElementById('username');
  const tag    = document.getElementById('usernameAutoTag');
  if (!uInput) return;
  // If user edits manually, stop auto-generation
  if (uInput.dataset.wasAuto) {
    delete uInput.dataset.wasAuto;
    _usernameAutoGenerated = false;
    if (tag) tag.style.display = 'none';
  }
  // Enforce English only
  const cleaned = uInput.value.replace(/[^A-Za-z0-9\-_.]/g, '');
  if (cleaned !== uInput.value) {
    uInput.value = cleaned;
    setHint('hintUsername', authLang === 'ar' ? 'أحرف إنجليزية فقط' : 'English characters only');
  } else {
    setHint('hintUsername', '');
  }
}
