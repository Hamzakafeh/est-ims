...
import time as _time_online

_online_sessions = {}          # session_id -> last_seen timestamp
_ONLINE_TIMEOUT  = 120         # ثانية — يوزر ما انبعث منه ping خلال دقيقتين = أوفلاين

@app.route('/api/online-count')
def online_count():
    """يُعاد تلقائياً كل 30 ثانية من welcome.html"""
    now = _time_online.time()
    # نظّف الجلسات المنتهية
    expired = [sid for sid, t in _online_sessions.items() if now - t > _ONLINE_TIMEOUT]
    for sid in expired:
        _online_sessions.pop(sid, None)

    # سجّل الزائر الحالي (نستخدم IP كـ identifier بسيط)
    visitor_id = request.headers.get('X-Forwarded-For', request.remote_addr).split(',')[0].strip()
    _online_sessions[visitor_id] = now

    return jsonify({'count': len(_online_sessions)})
...
CONTACT_RECEIVER = "hamzakafeh@gmail.com"

def _send_contact_email(name, phone, email, department, message):
    """إرسال رسالة الاتصال عبر Gmail SMTP."""
    sender       = os.getenv("GMAIL_USER")     # إيميل المرسل من .env
    app_password = os.getenv("GMAIL_APP_PASS") # App Password من .env
    if not sender or not app_password:
        _logger.warning("GMAIL_USER أو GMAIL_APP_PASS غير محدد في .env")
        return False

    msg = MIMEMultipart("alternative")
    msg["Subject"] = f"[EST-IMS] رسالة جديدة من {name}"
    msg["From"]    = sender
    msg["To"]      = CONTACT_RECEIVER

    body = f"""\
<html><body style="font-family:Arial,sans-serif;background:#f4f4f4;padding:24px;">
  <div style="max-width:520px;margin:auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.1);">
    <div style="background:linear-gradient(135deg,#3b82f6,#06b6d4);padding:20px 28px;">
      <h2 style="margin:0;color:#fff;font-size:18px;">📬 رسالة جديدة — EST-IMS</h2>
    </div>
    <div style="padding:24px 28px;">
      <table style="width:100%;border-collapse:collapse;font-size:14px;">
        <tr><td style="padding:8px 0;color:#666;width:120px;">الاسم</td><td style="padding:8px 0;font-weight:600;color:#1a1a1a;">{name}</td></tr>
        <tr><td style="padding:8px 0;color:#666;">الهاتف</td><td style="padding:8px 0;font-weight:600;color:#1a1a1a;">{phone}</td></tr>
        <tr><td style="padding:8px 0;color:#666;">الإيميل</td><td style="padding:8px 0;color:#1a1a1a;">{email or '—'}</td></tr>
        <tr><td style="padding:8px 0;color:#666;">القسم</td><td style="padding:8px 0;color:#1a1a1a;">{department or '—'}</td></tr>
      </table>
      <hr style="border:none;border-top:1px solid #eee;margin:16px 0;">
      <p style="color:#666;font-size:13px;margin:0 0 8px;">الرسالة:</p>
      <p style="background:#f8fafc;border-left:3px solid #3b82f6;padding:12px 16px;border-radius:0 8px 8px 0;color:#1a1a1a;margin:0;">{message}</p>
    </div>
    <div style="background:#f8fafc;padding:12px 28px;text-align:center;">
      <p style="margin:0;font-size:11px;color:#999;">EST-IMS · Alestesharia Animal Nutrition · {datetime.now().strftime('%Y-%m-%d %H:%M')}</p>
    </div>
  </div>
</body></html>"""
    ...
    try:
        with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
            ...
            server.sendmail(sender, CONTACT_RECEIVER, msg.as_string())
        return True
    except Exception as e:
        _logger.error("فشل إرسال الإيميل: %s", e)
        return False
...
def api_contact():
    ...
    # أرسل الإيميل في thread منفصل عشان ما يوقف الـ response
    threading.Thread(
        target=_send_contact_email,
        args=(name, phone, email, department, message),
        daemon=True
    ).start()
